...
const ATTACHMENT = b64encode(open("./528KB-random.png", "b"));
...
function sub(date: Date, years: number) {
...
export function createDeclaration(token: string) {
    ...
    JSON.stringify({
      ...
      variables: {
        details: {
          ...
          mother: {
            nationality: ["FAR"],
            birthDate: sub(birthDate, 20).toISOString().split("T")[0],
            name: [
              {
                use: "en",
